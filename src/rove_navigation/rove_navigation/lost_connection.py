        ...
        while True:
            ...
            try:
                ...
                socket.socket(socket.AF_INET, socket.SOCK_STREAM).connect(
                    (self.ADDRESS_TO_PING, self.PORT_TO_PING)
                )
                # if this following code runs, the connection was established
                self.nbFailedConns = 0
                if self.state != self.ONLINE:
                    self.get_logger().info(
                        "Connection re-established to server! terminating fallback state"
                    )
    ...
    def navigate_poses(self):
        ...
        if result == TaskResult.SUCCEEDED:
            ...
            self.get_logger().error(
                f"Couldn't proceed through fallbacks, reason unknown"
            )
        else:
            self.get_logger().warn(
                f"nav2 navigator task returned with unknown TaskResult"
            )
    ...
    def odometry_callback(self, msg: Odometry):
        ...
        # If robot at self.DISTANCE_BETWEEN_FALLBACKS distance or more from last fallback, add new fallback point
            ...
            dist = [
                ...
                - self.fallback_path.poses[0].pose.position.x,
                self.current_position.pose.position.y
                - self.fallback_path.poses[0].pose.position.y,
                self.current_position.pose.position.z
                - self.fallback_path.poses[0].pose.position.z,
            ]

            if np.linalg.norm(dist) >= self.DISTANCE_BETWEEN_FALLBACKS:
                self.fallback_path.poses.insert(0, self.current_position)
                self.get_logger().info(
                    f"New fallback position added: {self.fallback_path.poses[0].__str__()} total fallback points: {len(self.fallback_path.poses)}"
                )
