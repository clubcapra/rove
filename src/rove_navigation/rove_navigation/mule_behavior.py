...
class MuleBehavior(Node):
    def __init__(self):
        ...
        self.subscription_person_position = self.create_subscription(
            String, "/tracking/state", self.state_listener, 10
        )
        self.pub_mule_state = self.create_publisher(String, "/mule_state", 10)
        self.goal_update_pub = self.create_publisher(PoseStamped, "/goal_pose", 10)
        self.subscription_person_position = self.create_subscription(
            String, "/navigate_to_pose/_action/status", self.state_listener, 10
        )
        self.odom_subscriber = self.create_subscription(
            Odometry, "/odometry/local", self.odom_listener, 10
        )
            ...
            match msg.data:
                case consts.FIGURE_TPOSE:
                    if self.state == consts.START:
                        ...
                        self.get_logger().info(
                            f"Mule behavior engaged, point A noted at coordonates {self.startPoint.pose.position}"
                        )
                    returnState = consts.FOLLOWING

                case consts.FIGURE_IDLE:
                    if (
                        self.state != consts.REWIND
                        and self.state != consts.PROCESSED_REWIND
                    ):
                ...
                case consts.FIGURE_UP:
                    if (
                        ...
                        if self.state == consts.PAUSE:
                            returnState = consts.FOLLOWING
                        else:
                            returnState = consts.PAUSE
    ...
    def rewind_behavior(self):
        while self.state != consts.END:
            if self.state == consts.REACH_A:
                self.get_logger().info(
                    f"Going to point A coordonates {self.startpoint.pose.position}"
                )
            ...
            if self.state == consts.REACH_B:
                self.get_logger().info(
                    f"Going to point B coordonates {self.endpoint.pose.position}"
                )
