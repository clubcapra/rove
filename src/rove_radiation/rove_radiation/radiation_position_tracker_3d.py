...
class RadiationPositionTracker3D(Node):
    def __init__(self):
        ...
        self.map_size = 40
        self.radiation_map_local = np.zeros(
            (self.map_size, self.map_size, self.map_size), dtype=np.float32
        )
        ...
        for width in range(-self.radius, self.radius + 1):
            for height in range(-self.radius, self.radius + 1):
                for depth in range(-self.radius, self.radius + 1):
                    ...
                    if (
                        0 <= coord_x < self.map_size
                        and 0 < coord_z < self.map_size
                        and 0 < coord_y < self.map_size
                    ):
                        distance = math.sqrt(width ** 2 + height ** 2 + depth ** 2)
                        if distance <= self.radius:
                            attenuation = math.exp(-self.decay_factor * (distance // 4))
                            self.radiation_map_local[coord_x, coord_y, coord_z] = (
    ...
    def publish_point_cloud(self):
        ...
        for x in range(0, self.map_size, 5):
            for y in range(0, self.map_size, 5):
                for z in range(0, self.map_size, 5):
                    ...
                    real_x = (
                        x - self.map_size // 2
                    ) * self.resolution + self.current_position.x
                    real_y = (
                        y - self.map_size // 2
                    ) * self.resolution + self.current_position.y
                    real_z = (
                        z - self.map_size // 2
                    ) * self.resolution + self.current_position.z
                    ...
                    rgb = struct.unpack(
                        "f", struct.pack("I", (r_int << 16) | (g_int << 8) | b_int)
                    )[0]
        ...
        point_cloud_msg.height = (
            1  # 1 car c un nuage de pts organisé en ligne (pas en matrice)
        )
        ...
        point_cloud_msg.row_step = point_cloud_msg.point_step * len(
            rad_pos
        )  # la taille dune ligne complete
