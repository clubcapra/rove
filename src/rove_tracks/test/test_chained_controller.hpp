...
#ifndef TEMPLATES__ROS2_CONTROL__CONTROLLER__TEST_CHAINED_CONTROLLER_HPP_
#define TEMPLATES__ROS2_CONTROL__CONTROLLER__TEST_CHAINED_CONTROLLER_HPP_
...
using ControllerStateMsg = rove_tracks::ChainedController::ControllerStateMsg;
using ControllerReferenceMsg = rove_tracks::ChainedController::ControllerReferenceMsg;
using ControllerModeSrvType = rove_tracks::ChainedController::ControllerModeSrvType;
...
{
  FRIEND_TEST(ChainedControllerTest, all_parameters_set_configure_success);
  FRIEND_TEST(ChainedControllerTest, activate_success);
  FRIEND_TEST(ChainedControllerTest, reactivate_success);
  FRIEND_TEST(ChainedControllerTest, test_setting_slow_mode_service);
  FRIEND_TEST(ChainedControllerTest, test_update_logic_fast);
  FRIEND_TEST(ChainedControllerTest, test_update_logic_slow);
  FRIEND_TEST(ChainedControllerTest, test_update_logic_chainable_fast);
...
public:
  ...
  {
    auto ret = rove_tracks::ChainedController::on_configure(previous_state);
    ...
    const rclcpp_lifecycle::State & previous_state) override
  {
    ...
    return rove_tracks::ChainedController::on_activate(previous_state);
  }
...
class ChainedControllerFixture : public ::testing::Test
{
public:
  ...
  void SetUp()
  {
    ...
    command_publisher_ = command_publisher_node_->create_publisher<ControllerReferenceMsg>(
      "/test_chained_controller/commands", rclcpp::SystemDefaultsQoS());
    ...
    slow_control_service_client_ = service_caller_node_->create_client<ControllerModeSrvType>(
      "/test_chained_controller/set_slow_control_mode");
  }
...
protected:
  void SetUpController(const std::string controller_name = "test_chained_controller")
  {
  ...
  void subscribe_and_get_messages(ControllerStateMsg & msg)
  {
    ...
    auto subscription = test_subscription_node.create_subscription<ControllerStateMsg>(
      "/test_chained_controller/state", 10, subs_callback);
...
protected:
  ...
  std::unique_ptr<TestableChainedController> controller_;
...
#endif  // TEMPLATES__ROS2_CONTROL__CONTROLLER__TEST_CHAINED_CONTROLLER_HPP_
