...
from .canutils import CanBusCommandManager
...
class Bridge(Node):
    def __init__(self):
        ...
        self.canID = self.declare_parameter(
            'can_id', 1, ParameterDescriptor(type=ParameterType.PARAMETER_INTEGER, description='Can id'))
        ...
        self.manager.interface = 'socketcan'
        self.manager.channel = self.channel.value
        self.manager.localID = self.canID.value
        self.manager.remoteID = self.canID.value
        self.manager.bitrate = self.bitrate.value
...
def openSocket(bridge: Bridge):
    ...
    while True:
        ...
        try:
            ...
            manager.remoteID = bridge.canID.value
            manager.localID = bridge.canID.value
            manager.timeout = 2
            p = manager.ping()
